...
let displayStr = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames} games. Currently, ${numUnfunded} ${numUnfunded !== 1 ? "games" : "game"}
...
firstDiv.classList.add("first-game");
firstDiv.innerHTML = `<p>${first.name}</p>`;
firstGameContainer.appendChild(firstDiv);

// do the same for the runner up item
let secondDiv = document.createElement("div");
secondDiv.classList.add("second-game");
secondDiv.innerHTML = `<p>${second.name}</p>`;
secondGameContainer.appendChild(secondDiv);